...
function BitStream() {
    ...
    function putbits2(gfc, val, j) {
        ...
        while (j > 0) {
            ...
            if (bufBitIdx == 0) {
                bufBitIdx = 8;
                bufByteIdx++;
                assert(bufByteIdx < constant.Lame.LAME_MAXMP3BUFFER);
        ...
        if (gfp.version == 1) {
            ...
            for (gr = 0; gr < 2; gr++) {
                for (ch = 0; ch < gfc.channels_out; ch++) {
                    ...
                    if (gi.block_type != constant.Encoder.NORM_TYPE) {
                        writeheader(gfc, 1, 1);
    ...
    function writeMainData(gfp) {
        ...
        if (gfp.version == 1) {
            ...
            for (gr = 0; gr < 2; gr++) {
                for (ch = 0; ch < gfc.channels_out; ch++) {
                    var gi = l3_side.tt[gr][ch];
                    var slen1 = constant.Takehiro.slen1_tab[gi.scalefac_compress];
                    var slen2 = constant.Takehiro.slen2_tab[gi.scalefac_compress];
                    data_bits = 0;
                    ...
                    if (gi.block_type == constant.Encoder.SHORT_TYPE) {
    ...
    this.init_bit_stream_w = function (gfc) {
        buf = new_byte(constant.Lame.LAME_MAXMP3BUFFER);
